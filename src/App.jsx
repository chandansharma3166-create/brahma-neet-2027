...
export default function App() {
  ...
  // ================= 1. AUTHENTICATION & MULTI-USER STATE =================
  ...
  // Multi-tier Analytics Scope
  const [analyticsScope, setAnalyticsScope] = useState('7');

  // Timers State (Absolute Timestamps for cross-device & refresh persistence)
  ...
  useEffect(() => {
    ...
    try {
      ...
      setMockTests(savedMocks ? JSON.parse(savedMocks) : [
        { id: 1, name: 'Major Test 1', date: '2026-08-10', timeframe: 'Monthly', physics: 130, chemistry: 140, biology: 310, total: 580, negativeMarks: 20, rank: 350 },
        { id: 2, name: 'Major Test 2', date: '2026-08-17', timeframe: 'Weekly', physics: 140, chemistry: 145, biology: 325, total: 610, negativeMarks: 16, rank: 240 },
        { id: 3, name: 'Major Test 3', date: '2026-08-24', timeframe: 'Weekly', physics: 150, chemistry: 155, biology: 335, total: 640, negativeMarks: 12, rank: 160 }
      ]);
  ...
  // Subject Timer Completion Calculators
  const getSubjectTimerStats = () => {
    const calcStat = (id) => {
      const slot = slots.find(s => s.id === id);
      if (!slot) return { percent: 0, completedMins: 0, targetMins: 0 };
      const totalSec = slot.durationMinutes * 60;
      const spentSec = Math.max(0, totalSec - slot.timeLeft);
      const percent = Math.min(100, Math.round((spentSec / totalSec) * 100));
      return { percent, completedMins: Math.round(spentSec / 60), targetMins: slot.durationMinutes };
    };
  ...
  const subjectTimerStats = getSubjectTimerStats();
  const overallTimerProgress = Math.round(
    (subjectTimerStats.phy.percent + subjectTimerStats.chem.percent + subjectTimerStats.bio.percent + subjectTimerStats.rev.percent) / 4
  );

  // ================= 3. BULLETPROOF TASK HANDLERS =================
  ...
  const addTask = (e) => {
    e.preventDefault();
    if (!newTaskInput.text || !newTaskInput.text.trim()) return;
    const newTask = {
      id: Date.now() + Math.random(),
      date: selectedTaskDate,
      text: newTaskInput.text.trim(),
      timeSlot: newTaskInput.timeSlot.trim() || 'Flexible',
      completed: false
    };
  ...
  // ================= 4. UNIFIED ANALYTICS ENGINE (BMI) =================
  const generateUnifiedAnalytics = () => {
    const dateMap = {};

    (dailyTasks || []).forEach(task => {
      const d = task.date || todayDateStr;
      if (!dateMap[d]) dateMap[d] = { date: d, taskTotal: 0, taskCompleted: 0, dwarScore: 0, mockScore: 0 };
      dateMap[d].taskTotal += 1;
      if (task.completed) dateMap[d].taskCompleted += 1;
    });

    (dwarLogs || []).forEach(dwar => {
      const d = dwar.date || todayDateStr;
      if (!dateMap[d]) dateMap[d] = { date: d, taskTotal: 0, taskCompleted: 0, dwarScore: 0, mockScore: 0 };
      dateMap[d].dwarScore = Number(dwar.score) || 0;
    });

    (mockTests || []).forEach(mock => {
      const d = mock.date || todayDateStr;
      if (!dateMap[d]) dateMap[d] = { date: d, taskTotal: 0, taskCompleted: 0, dwarScore: 0, mockScore: 0 };
      dateMap[d].mockScore = Math.min(100, Math.round((Number(mock.total || 0) / 720) * 100));
    });

    let datesArray = Object.values(dateMap).map(item => {
      const taskRate = item.taskTotal > 0 ? Math.round((item.taskCompleted / item.taskTotal) * 100) : 0;
      const dwarRate = (item.dwarScore / 10) * 100;
      const bmi = Math.round((taskRate * 0.35) + (overallTimerProgress * 0.35) + (dwarRate * 0.20) + (item.mockScore * 0.10));

      return {
        ...item,
        taskRate: Number(taskRate),
        dwarRate: Number(dwarRate),
        timerRate: Number(overallTimerProgress),
        bmi: Number(bmi)
      };
    }).sort((a, b) => new Date(a.date) - new Date(b.date));

    if (analyticsScope === '1') datesArray = datesArray.slice(-1);
    else if (analyticsScope === '4') datesArray = datesArray.slice(-4);
    else if (analyticsScope === '7') datesArray = datesArray.slice(-7);
    else if (analyticsScope === '30') datesArray = datesArray.slice(-30);

    const totalTasks = datesArray.reduce((acc, c) => acc + c.taskTotal, 0);
    const completedTasks = datesArray.reduce((acc, c) => acc + c.taskCompleted, 0);
    const avgBMI = datesArray.length > 0 ? Math.round(datesArray.reduce((acc, c) => acc + c.bmi, 0) / datesArray.length) : 0;
    const avgTaskRate = totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0;

    return { datesArray, totalTasks, completedTasks, avgBMI, avgTaskRate };
  };

  const unifiedAnalytics = generateUnifiedAnalytics();
  ...
  // Mock Helpers & Fixed Clean Rendering
  ...
  const filteredMockTests = (mockTests || []).filter(m => {
    ...
    physics: Number(m.physics || 0),
    chemistry: Number(m.chemistry || 0),
    biology: Number(m.biology || 0)
  }));
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      <header className="border-b border-slate-800 bg-slate-900/60 backdrop-blur-md sticky top-0 z-40 px-4 py-3 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button 
            ...
            title="Enable Device Notification Alarms"
          >
      ...
      <nav className="flex space-x-1 p-2 bg-slate-900 border-b border-slate-800 text-sm overflow-x-auto">
        {[
          ...
          { id: 'analytics', label: 'Unified Mastery Analytics', icon: Activity },
      ...
      <main className="flex-1 p-4 md:p-6 max-w-6xl w-full mx-auto space-y-6">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex flex-col justify-between">
                <div>
                  ...
                  <div className="flex items-baseline space-x-2 mt-2">
                    ...
                    <span className="text-xs text-slate-400">({completedTodayCount}/{displayedTasks.length})</span>
                  </div>
                </div>
              ...
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs uppercase font-bold text-slate-400">Timers Score</span>
                    <span className="text-xs text-emerald-400 font-mono font-bold">{overallTimerProgress}% Overall</span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 mt-2 text-[10px] font-mono">
                    <div className="bg-slate-950/60 p-1.5 rounded-lg border border-slate-800 text-center">
                      <span className="text-slate-400 block">PHY</span>
                      <span className="font-bold text-blue-400">{subjectTimerStats.phy.percent}%</span>
                    </div>
                    <div className="bg-slate-950/60 p-1.5 rounded-lg border border-slate-800 text-center">
                      <span className="text-slate-400 block">CHEM</span>
                      <span className="font-bold text-emerald-400">{subjectTimerStats.chem.percent}%</span>
                    </div>
                    <div className="bg-slate-950/60 p-1.5 rounded-lg border border-slate-800 text-center">
                      <span className="text-slate-400 block">BIO</span>
                      <span className="font-bold text-purple-400">{subjectTimerStats.bio.percent}%</span>
                    </div>
                  </div>
                </div>
                <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden mt-3">
                  <div className="bg-emerald-500 h-full rounded-full transition-all duration-500" style={{ width: `${overallTimerProgress}%` }} />
                </div>
              </div>
              ...
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs uppercase font-bold text-slate-400">SM-2 Due</span>
                  ...
                  <div className="mt-2 space-y-1 max-h-16 overflow-y-auto">
                    {dueRevisionsToday.length === 0 ? (
                      <p className="text-xs text-slate-500">All SM-2 cards cleared!</p>
                    ) : (
                      dueRevisionsToday.slice(0, 2).map(r => (
                        <div key={r.id} className="text-[11px] truncate text-slate-300">
                          • {r.topic} ({r.subject})
                        </div>
                      ))
                    )}
              ...
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex flex-col justify-between">
                <div>
                  <span className="text-xs uppercase font-bold text-slate-400">D.W.A.R. Status</span>
                  <p className="text-xs text-slate-300 mt-2 line-clamp-2">
                    {dwarLogs.find(d => d.date === todayDateStr)?.did || 'No DWAR self-analysis submitted yet.'}
                  </p>
                </div>
            ...
            {/* Daily Execution Checklist Form & List */}
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
              ...
              {/* Verified Working Add Task Form */}
              <form onSubmit={addTask} className="flex gap-2 text-xs">
                <input 
                  type="text" 
                  required
                  placeholder={`Add specific task for ${selectedTaskDate}...`} 
                  value={newTaskInput.text}
                  onChange={(e) => setNewTaskInput({ ...newTaskInput, text: e.target.value })}
                  className="flex-1 bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white outline-none focus:border-amber-500"
                />
                <input 
                  type="text" 
                  ...
                  className="w-44 bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white hidden sm:block outline-none focus:border-amber-500"
                />
                <button type="submit" className="bg-amber-600 hover:bg-amber-500 text-white px-4 py-2 rounded-xl font-semibold flex items-center space-x-1 shadow-md shadow-amber-600/20">
              ...
              <div className="divide-y divide-slate-800/60">
                {displayedTasks.length === 0 ? (
                  <p className="text-xs text-slate-500 text-center py-4">No tasks recorded for {selectedTaskDate}. Add your first goal above!</p>
                ) : (
        ...
        {/* TAB 1: UNIFIED MASTERY ANALYTICS */}
        {activeTab === 'analytics' && (
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h2 className="text-lg font-bold flex items-center space-x-2">
                  ...
                  <span>Comprehensive NEET Performance & Mastery Telemetry</span>
                </h2>
                <p className="text-xs text-slate-400">Cross-analyzing Tasks + Subject Timer Ratios + DWAR Rating + Mock Scores</p>
              </div>
              ...
              <div className="flex flex-wrap bg-slate-900 border border-slate-800 p-1 rounded-xl text-xs gap-1">
                {[
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg font-bold transition ${analyticsScope === scope.id ? 'bg-amber-600 text-white' : 'text-slate-400 hover:text-white'}`}
                  >
                    {scope.label}
                  </button>
                ))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl text-center">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Brahma Mastery Index (BMI)</span>
                <h3 className="text-3xl font-extrabold font-mono text-amber-400 mt-2">{unifiedAnalytics.avgBMI}%</h3>
                <p className="text-[11px] text-slate-500 mt-1">Weighted composite metric</p>
              </div>

              <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl text-center">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Task Completion Rate</span>
                <h3 className="text-3xl font-extrabold font-mono text-emerald-400 mt-2">{unifiedAnalytics.avgTaskRate}%</h3>
                <p className="text-[11px] text-slate-500 mt-1">{unifiedAnalytics.completedTasks}/{unifiedAnalytics.totalTasks} cleared</p>
              </div>

              <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl text-center">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Timer Allocation Score</span>
                <h3 className="text-3xl font-extrabold font-mono text-cyan-400 mt-2">{overallTimerProgress}%</h3>
                <p className="text-[11px] text-slate-500 mt-1">Subject study target reached</p>
              </div>

              <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl text-center">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Logged Span</span>
                <h3 className="text-3xl font-extrabold font-mono text-purple-400 mt-2">{unifiedAnalytics.datesArray.length} Days</h3>
                <p className="text-[11px] text-slate-500 mt-1">Active analysis window</p>
              </div>
            </div>

            {/* Dual Multi-Factor Telemetry Charts */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3">
                <h3 className="text-sm font-bold text-slate-200">Holistic Mastery Index (BMI) & Task Trends (%)</h3>
                <div className="h-64 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={unifiedAnalytics.datesArray}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                      <XAxis dataKey="date" stroke="#64748b" fontSize={10} />
                      <YAxis domain={[0, 100]} stroke="#64748b" fontSize={10} unit="%" />
                      <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '12px' }} />
                      <Legend />
                      <Bar dataKey="taskRate" name="Tasks %" fill="#3b82f6" radius={[4, 4, 0, 0]} />
                      <Line type="monotone" dataKey="bmi" name="Brahma Index %" stroke="#f59e0b" strokeWidth={3} dot={{ r: 4 }} />
                      <Line type="monotone" dataKey="dwarRate" name="DWAR Rating %" stroke="#10b981" strokeWidth={2} strokeDasharray="3 3" />
                    </ComposedChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3">
                <h3 className="text-sm font-bold text-slate-200">Subject Time Investment & Completion Ratios</h3>
                <div className="space-y-4 pt-2">
                  <div>
                    <div className="flex justify-between text-xs font-semibold mb-1">
                      <span className="text-blue-400">Physics Block (2.5h Target)</span>
                      <span className="font-mono">{subjectTimerStats.phy.percent}% ({subjectTimerStats.phy.completedMins}/{subjectTimerStats.phy.targetMins}m)</span>
                    </div>
                    <div className="w-full bg-slate-800 h-2.5 rounded-full overflow-hidden">
                      <div className="bg-blue-500 h-full rounded-full transition-all" style={{ width: `${subjectTimerStats.phy.percent}%` }} />
                    </div>
                  </div>

                  <div>
                    <div className="flex justify-between text-xs font-semibold mb-1">
                      <span className="text-emerald-400">Chemistry Block (2.0h Target)</span>
                      <span className="font-mono">{subjectTimerStats.chem.percent}% ({subjectTimerStats.chem.completedMins}/{subjectTimerStats.chem.targetMins}m)</span>
                    </div>
                    <div className="w-full bg-slate-800 h-2.5 rounded-full overflow-hidden">
                      <div className="bg-emerald-500 h-full rounded-full transition-all" style={{ width: `${subjectTimerStats.chem.percent}%` }} />
                    </div>
                  </div>

                  <div>
                    <div className="flex justify-between text-xs font-semibold mb-1">
                      <span className="text-purple-400">Biology Block (1.5h Target)</span>
                      <span className="font-mono">{subjectTimerStats.bio.percent}% ({subjectTimerStats.bio.completedMins}/{subjectTimerStats.bio.targetMins}m)</span>
                    </div>
                    <div className="w-full bg-slate-800 h-2.5 rounded-full overflow-hidden">
                      <div className="bg-purple-500 h-full rounded-full transition-all" style={{ width: `${subjectTimerStats.bio.percent}%` }} />
                    </div>
                  </div>

                  <div>
                    <div className="flex justify-between text-xs font-semibold mb-1">
                      <span className="text-amber-400">Targeted Revision Block (2.0h Target)</span>
                      <span className="font-mono">{subjectTimerStats.rev.percent}% ({subjectTimerStats.rev.completedMins}/{subjectTimerStats.rev.targetMins}m)</span>
                    </div>
                    <div className="w-full bg-slate-800 h-2.5 rounded-full overflow-hidden">
                      <div className="bg-amber-500 h-full rounded-full transition-all" style={{ width: `${subjectTimerStats.rev.percent}%` }} />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-3">
              <div className="flex justify-between items-center">
                <h3 className="text-sm font-bold text-slate-200">Historical Multi-Factor Matrix</h3>
                <span className="text-xs text-slate-400">{unifiedAnalytics.datesArray.length} records in view</span>
              </div>
                ...
                <table className="w-full text-left text-xs text-slate-300">
                  <thead className="bg-slate-800/60 uppercase tracking-wider text-slate-400 font-semibold border-b border-slate-800">
                    <tr>
                      ...
                      <th className="p-3">Task Ratio</th>
                      <th className="p-3">DWAR Rating</th>
                      <th className="p-3">Mock Score</th>
                      <th className="p-3">Brahma Index (BMI)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800/60">
                    {unifiedAnalytics.datesArray.length === 0 ? (
                      <tr><td colSpan="5" className="p-6 text-center text-slate-500">No telemetry logged in this interval.</td></tr>
                    ) : (
                      unifiedAnalytics.datesArray.slice().reverse().map(item => (
                        <tr key={item.date} className="hover:bg-slate-800/30 transition">
                          ...
                          <td className="p-3 font-mono">{item.taskCompleted}/{item.taskTotal} ({item.taskRate}%)</td>
                          <td className="p-3 font-mono text-emerald-400 font-bold">{item.dwarScore}/10</td>
                          <td className="p-3 font-mono text-blue-400">{item.mockScore > 0 ? `${item.mockScore}%` : 'N/A'}</td>
                          <td className="p-3">
                            <span className={`px-2 py-0.5 rounded-full font-mono font-bold text-[10px] ${
                              item.bmi >= 75 ? 'bg-emerald-950 text-emerald-400 border border-emerald-800/50' :
                              item.bmi >= 50 ? 'bg-amber-950 text-amber-400 border border-amber-800/50' :
                    ...
                    {!sheetSchedule || sheetSchedule.length === 0 ? (
                      <tr>
                        <td colSpan="6" className="p-8 text-center text-slate-500">
                          {isSyncing ? 'Fetching syllabus from Google Sheets...' : 'No Google Sheet schedule loaded yet. Click "Fetch Cloud" or "Backup Sheet" above.'}
                        </td>
                      </tr>
                    ) : (
        ...
        {/* TAB 6: MOCK TELEMETRY & GRAPH RENDERING */}
        {activeTab === 'mock' && (
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h2 className="text-lg font-bold flex items-center space-x-2">
                  ...
                  <span>Mock Telemetry, Rank Tracking & Score Progression</span>
                </h2>
                <p className="text-xs text-slate-400">Weekly and monthly progression curves toward 700+</p>
              </div>
              <div className="flex items-center space-x-3">
                <div className="flex bg-slate-900 border border-slate-800 p-1 rounded-xl text-xs">
                  {['All', 'Weekly', 'Monthly'].map(f => (
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg font-bold transition ${mockFilter === f ? 'bg-amber-600 text-white' : 'text-slate-400 hover:text-white'}`}
                    >
                      {f}
                    </button>
                  ))}
                </div>
                <button onClick={() => setIsMockModalOpen(true)} className="flex items-center space-x-1.5 bg-amber-600 hover:bg-amber-500 text-white px-3.5 py-2 rounded-xl text-xs font-medium transition shadow-lg shadow-amber-600/20">
                  <PlusCircle className="w-4 h-4" />
                  <span>Log Test</span>
                </button>
              </div>
                ...
                <div className="h-64 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={filteredMockTests}>
                      ...
                      <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '12px' }} />
                      <Legend />
                      <Line type="monotone" dataKey="total" name="Total Marks" stroke="#f59e0b" strokeWidth={3} dot={{ r: 5 }} isAnimationActive={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>

              {/* Projected Rank Line Chart */}
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3">
                ...
                <div className="h-64 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={filteredMockTests}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                      <XAxis dataKey="name" stroke="#64748b" fontSize={11} />
                      <YAxis reversed stroke="#64748b" fontSize={11} domain={['auto', 'auto']} />
                      <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '12px' }} />
                      <Legend />
                      <Line type="monotone" dataKey="rank" name="Projected Rank" stroke="#10b981" strokeWidth={3} dot={{ r: 5 }} isAnimationActive={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
            ...
            <form onSubmit={logNewAttempt} className="space-y-4 text-xs">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl p-2.5 text-white font-mono text-base outline-none focus:border-amber-500"
                />
              </div>
